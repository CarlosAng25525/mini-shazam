...
class HashTable:
    ...
    def lookup(self, key):
        ...
        """
        return [value for (stored_key, value) in self._buckets[self._hash(key)] if stored_key == key]
    ...
    def stats(self):
        ...
        return {
            ...
            "empty_buckets": sum(1 for bucket in self._buckets if len(bucket) == 0),
            "max_chain_length": max(len(bucket) for bucket in self._buckets),
            "avg_chain_length": round(avg_chain_length / self.capacity, 4) 
        }
    # ------------------------------------------------------------------ #
    # Resizing
    # ------------------------------------------------------------------ #
    ...
    def _next_prime(n):
        ...
        if prime % 2 == 0:
            prime += 1
        for i in range(3, math.floor(math.sqrt(prime)) + 1, 2):
            if prime % i == 0:
                prime += 2
        return prime
